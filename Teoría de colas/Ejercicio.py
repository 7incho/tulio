...
def VectorInicial():
    global Q, PS, T, P, tci, tcf, tai, taf, caso, S, tdi, tdf, tti, ttf, header, SigFinServicio, SigLlegada, SigLlegadaS, SigSalidaS, HF, HA, abandono_cliente, tiempo_abandono
    ...
    abandono_cliente = int(input("¿Los clientes pueden abandonar la cola? (Si = 1; No = 0): "))
    if abandono_cliente == 1:
        tiempo_abandono = int(input("Ingrese el tiempo en minutos después del cual un cliente abandona la cola: "))

def LlegadaCliente():
    a = random.randint(tai, taf)
    b = random.randint(tci, tcf)
    global Q, PS, HA, SigFinServicio, SigLlegada, abandono_cliente, tiempo_abandono
    if PS == 0 and S == 1:
        PS = 1
        SigFinServicio = HA + dt.timedelta(minutes=a)
    else:
        Q = Q + 1
        if abandono_cliente == 1 and Q >= 1 and (HA - SigLlegada).total_seconds() / 60 >= tiempo_abandono:
            Q = Q - 1
...
def AbandonoCliente():
    global Q, HA, SigLlegada, abandono_cliente, tiempo_abandono
    if abandono_cliente == 1 and Q >= 1 and (HA - SigLlegada).total_seconds() / 60 >= tiempo_abandono:
        Q = Q - 1
    ...
    if caso == 0:
        ...
        while True:
            ...
            if min(SigFinServicio, SigLlegada, SigLlegadaS, SigSalidaS) == SigFinServicio:
                ...
                break
            AbandonoCliente()
            print(HA, "        ", SigLlegada, "                 ", SigFinServicio, "              ", SigSalidaS,
                  "                ", SigLlegadaS, "                   ", Q, "  ", PS, "  ", S)
            if HA >= HF:
                print("-------------- Fin de la simulación -------------- ")
                break
